...
namespace YQDHWebsite.DB {
    /// <summary>
    /// SQLServerHelper工具类，用于查询船舶网新闻动态接口 
    /// </summary>
    public sealed class SQLServerHelper
    {
        ...
        /// 主数据库链接串
        /// </summary>
        public static readonly string BaseCon = ConfigurationManager.AppSettings["BaseCon"];

        #region private utility methods & constructors

        // Since this class provides only static methods, make the default constructor private to prevent 
        // instances from being created with "new SQLServerHelper()"
        private SQLServerHelper() { }

        #region 批量执行sql语句
        public static int BatchExecuteBySql(string connectionString, List<String> lstSQL)
        {
            int num = 0;
            int totalnum = 0;
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    try
                    {
                        foreach (string szSQL in lstSQL)
                        {
                            if (szSQL != null)
                            {
                                num = ExecuteNonQuery(connectionString, CommandType.Text, szSQL);
                                totalnum += num;
                            }
                        }
                        transaction.Commit();
        ...
        public static int BatchExecuteBySql(string connectionString, Hashtable ht)
        {
            int num = 0;
            int totalnum = 0;
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    try
                    {
                        ...
                        {
                            string sql = ht[key].ToString();
                            if (sql != null)
                            {
                                num = ExecuteNonQuery(connectionString, CommandType.Text, sql);
                                totalnum += num;
                            }
                        }
                        transaction.Commit();
        ...
        private static void PrepareCommand(SqlCommand command, SqlConnection connection, SqlTransaction transaction, CommandType commandType, string commandText, SqlParameter[] commandParameters, bool IsTimeOut, out bool mustCloseConnection)
        {
            if (command == null) throw new ArgumentNullException("command");
        ...
        {
            // Pass through the call providing null for the set of SqlParameters
            return ExecuteNonQuery(BaseCon, CommandType.Text, commandText, commandParameters);
        }
        ...
        {
            return ExecuteDataset(BaseCon, CommandType.Text, commandText, (SqlParameter[])null);
        }
        public static DataTable ExecuteDt(string commandText)
        {
            return ExecuteDataset(commandText).Tables[0];
        }
        public static DataTable ExecuteDtTxt(string commandText, params SqlParameter[] commandParameters)
        {
            return ExecuteDataset(BaseCon, CommandType.Text, commandText, commandParameters).Tables[0];
        }

        public static DataTable ExecuteDt(string commandText, params SqlParameter[] commandParameters)
        {
            return ExecuteDataset(BaseCon, commandText, commandParameters).Tables[0];
        }

        public static DataSet ExecuteDataset(SqlConnection connection, CommandType commandType, string commandText, bool IsTimeOut, params SqlParameter[] commandParameters)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            ...
            SqlCommand cmd = new SqlCommand();
            bool mustCloseConnection = false;
            PrepareCommand(cmd, connection, (SqlTransaction)null, commandType, commandText, commandParameters, IsTimeOut, out mustCloseConnection);
    ...
    public sealed class SQLServerHelperParameterCache
    {
        ...
        //Since this class provides only static methods, make the default constructor private to prevent 
        //instances from being created with "new SQLServerHelperParameterCache()"
